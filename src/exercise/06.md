...
I learnt to represent forms in React, in particular by defining submit handlers and change handlers to catch particular events and act consequently.

I learnt how to use refs in React, by using the ref attribute of the input tag to get the value, and also useState (i.e., defining a couple 'x, setX' and setting it in the on change handler). The button tag has a 'disabled' attribute that prevents it to be clicked. Finally, useState can also be used to change the input prompted by the user by setting the value attribute of the input tag.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=alessandrosommarug%40gmail.com).
