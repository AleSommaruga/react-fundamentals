...
In order to write react components, you must first create some functions (with capital letter), then call them by putting it inside an HTML tag with the function's parameters as attributes. You can use propTypes to check that the attributes are provided and they are of the correct type, you can also use React.Fragment (or <> </>) to avoid having a container class as parent element.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=alessandrosommarug%40gmail.com).
